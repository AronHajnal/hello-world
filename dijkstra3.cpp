...
int inf = 100;//std::numeric_limits<int>::max();
...
class Graph{
    ...
    std::vector<int> neighbours(int x){
        std::vector<int> out;
        for(unsigned int i = 0; i<= matrix[x].size();i++){
            if(matrix[x][i] > 0 && matrix[x][i] < inf){
                out.push_back(i);
                }
            }
        return out;
    ...
    void add_node(){
        ...
        std::vector<int> temp (matrix.size(), inf);
        temp.push_back(0);
        matrix.push_back(temp);
        }
...
class Dijkstra{
    ...
    public:
    Dijkstra(Graph g): graph(g), heap{}, distances(g.num_vert(), std::vector<int>{}){}
    std::vector<int> shortest(int source){
        std::vector<int> d = distances[source];
        ...
        while(heap.size() != 0){
            Node current = heap.min_priority();
            for(auto i:graph.neighbours(current.vertex)){
                if(d.at(current.vertex)+i < d.at(i)){
                    d.at(i) = d.at(current.vertex)+i;
                    }
                }
            }
        return d;
